...
public class VerifyStoreCommandHandlerTests
{
	private readonly Mock<IStoreRepository> _storeRepository = new();
	private readonly Mock<IUnitOfWork> _unitOfWork = new();
	private readonly Mock<ILogger<VerifyStoreCommandHandler>> _logger = new();

	private VerifyStoreCommandHandler CreateSut()
	...
	public async Task Handle_WhenStoreNotFound_ReturnsFailure()
	{
		...
		var sut = CreateSut();
		var cmd = new VerifyStoreCommand(storeId);
	...
	[Fact]
	public async Task Handle_WhenValidRequest_VerifiesStore()
	{
		// Arrange
		...
		var sut = CreateSut();
		var cmd = new VerifyStoreCommand(store.Id);
		...
		res.Message.Should().Be("Store verified successfully");
